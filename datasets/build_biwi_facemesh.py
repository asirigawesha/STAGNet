"""Build BIWI FaceMesh NPZ files for STAGNet Protocol II experiments.
...
FACEMESH_TASK = Path("face_landmarker.task")
...
def build_split(subject_list, detector, ssd_net, name="split"):
    ...
    for subject_dir in subject_list:
        ...
        for j in tqdm(range(len(pngs))):
            ...
            lms = np.array([[lm.x, lm.y, lm.z] for lm in result.face_landmarks[0]], dtype=np.float32)
            out_lms.append(lms)
...
def main():
    if not FACEMESH_TASK.exists():
        raise FileNotFoundError(f"Missing FaceMesh task model: {FACEMESH_TASK.resolve()}")
    ...
    options = vision.FaceLandmarkerOptions(
        base_options=python.BaseOptions(model_asset_path=str(FACEMESH_TASK)),
        num_faces=1,
    ...
    with vision.FaceLandmarker.create_from_options(options) as detector:
        train_lms, train_poses = build_split(train_subjects, detector, ssd_net, name="train")
        test_lms, test_poses = build_split(test_subjects, detector, ssd_net, name="test")
    ...
    train_lms_norm = normalize_landmarks(train_lms, anchor_index=1)
    test_lms_norm = normalize_landmarks(test_lms, anchor_index=1)
    all_lms_norm = normalize_landmarks(all_lms, anchor_index=1)

    landmark_stats(all_lms_norm, name="all")
    landmark_stats(train_lms_norm, name="train")
    landmark_stats(test_lms_norm, name="test")

    out_all = Path(f"{OUT_PREFIX}_facemesh_all.npz")
    out_train = Path(f"{OUT_PREFIX}_facemesh_train.npz")
    out_test = Path(f"{OUT_PREFIX}_facemesh_test.npz")
